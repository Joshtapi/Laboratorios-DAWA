...
const fileFilter = (req, file, cb) => {
    // Validar el tipo de archivo permitido (imágenes, documentos de texto, pdf)
    if (file.mimetype.startsWith('image/') || file.mimetype === 'application/pdf' || file.mimetype === 'text/plain') {
        cb(null, true);
    } else {
        cb(new Error('Tipo de archivo no permitido. Solo se permiten imágenes, documentos de texto o archivos PDF.'));
    }
};
